...
class SnakePlayer(list):
	...
	def eyes(self):
		...
		return  [self.direction==a for a in [0,1,2,3]] + [foodD==a for a in [0,1,2,3]]+ self.sonar()
	...
	def sonar(self):
		...
		x = self.body[0][0]
		y = self.body[0][1]
		A = [theMap[min(x+1,14),y],theMap[max(x-1,0),y],
theMap[x,min(y+1,14)],theMap[x,max(y-1,0)],
theMap[min(x+1,14),min(y+1,14)],theMap[max(x-1,0),min(y+1,14)],theMap[min(x+1,14),max(y-1,0)],theMap[max(x-1,0),max(y-1,0)]]
		B = [x+1==15,x-1==-1,y+1==15,y-1==-1,0,0,0,0]
