...
class SnakePlayer(list):
	...
	def eyes(self):
		...
		return  [self.direction==a for a in [0,1,2,3]] + [foodD==a for a in [0,1,2,3]] + distance_to_wall + self.sonar()
	...
	def sonar(self):
		...
		return [theMap[min(x+1,14),y],theMap[max(x-1,0),y],theMap[x,min(y+1,14)],theMap[x,max(y-1,0)]]
